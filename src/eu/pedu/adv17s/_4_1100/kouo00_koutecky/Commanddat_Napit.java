...
//\II== INSTANCE INITIALIZERS (CONSTRUCTORS) ===================================
     ...
     */
    public Commanddat_Napit()
    {
        super ("dat_napit", "date napit blondynce");
    }
...
//\IM== INSTANCE REMAINING NON-PRIVATE METHODS =================================
    ...
    public String execute(String... arguments)
    {
        if (arguments.length <3) {
            return "spatny format prikazu, napiste: dat_Napit [napoj] [komu]";
        }
        
        if (!arguments[2].matches("blondynka")) {
            return arguments[2] + "neumi pit!, zvolte jineho komzumenta";
        }
        
        
        Room postava = Potion.getInstance().getCurrentPlace();
        Thing blondynka = postava.getObject("blondynka");
        if (arguments[1].matches("lektvar")) {
            if(blondynka == null){
            return "v tomto prostoru neni blondynka";
            }
            else
            {
            postava.removeObject(blondynka);
            Thing postava1 = new Thing ("#zamilovana_blondynka");
           
            postava.addObject (postava1);
                   
            String lahev = "lektvar";
            Bag batoh = Bag.getInstance();
            Thing object = batoh.getObject(lahev);
            batoh.removeObject (object);
            Thing object1 = new Thing("_lahvicka");
            batoh.addObject (object1);
            return "dali jste napit lektvar blondynce."; 
            }
            
        }
        return "nelze ji dat napit " + arguments[1];
